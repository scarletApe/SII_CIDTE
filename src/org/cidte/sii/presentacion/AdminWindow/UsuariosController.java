...
public class UsuariosController implements Initializable {
	...
	void handleGuardar(ActionEvent event) {
		...
		if (curp.isEmpty() || nu.isEmpty() || contrasena.isEmpty() || rol.isEmpty()) {
			return;
		}
	...
	private void createTable() {
		...
		colID.setCellValueFactory(new PropertyValueFactory<>("curp"));
		...
		colNU.setCellValueFactory(new PropertyValueFactory<>("username"));
		iv = new ImageView(gato);
		iv.setFitHeight(30);
		iv.setFitWidth(30);
		colNU.setGraphic(iv);
		...
		tablaUsuarios.getColumns().addAll(colID, colNU, colRol, colPass);
	}
	...
	private void fillTable() {
		...
		ObservableList<String> roles = FXCollections.observableArrayList("Adm", "Reg");
